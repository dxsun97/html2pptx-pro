...
describe('property-descriptors', () => {
    ...
    beforeEach(() => {
        ...
        const config = new PptxConfig({ window: mockWindow });
    ...
    describe('background-image', () => {
        ...
        it(`linear-gradient(to bottom, rgba(255,255,0,0.5), rgba(0,0,255,0.5)), url('https://example.com')`, () =>
            deepStrictEqual(
                backgroundImageParse(
                    context,
                    `linear-gradient(to bottom, rgba(255,255,0,0.5), rgba(0,0,255,0.5)), url('https://example.com')`
                ),
                [
                    {
                        angle: deg(180),
                    ...
                    { url: 'https://example.com', type: CSSImageType.URL }
                ]
            ));
    });
});
