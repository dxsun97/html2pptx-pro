 ...
 * The optional `round` values follow the border-radius 1-4 value shorthand.
 */
export interface InsetClipPath {
    ...
    round: [LengthPercentage, LengthPercentage, LengthPercentage, LengthPercentage] | null;
}
 ...
 * The optional `round` clause follows the border-radius 1-4 value shorthand.
 */
const parseInset = (values: CSSValue[]): InsetClipPath => {
    const lengths: LengthPercentage[] = [];
    const roundLengths: LengthPercentage[] = [];
    ...
    let round: InsetClipPath['round'] = null;
    if (roundLengths.length > 0) {
        const r0 = roundLengths[0];
        const r1 = roundLengths[1] ?? r0;
        const r2 = roundLengths[2] ?? r0;
        const r3 = roundLengths[3] ?? r1;
        round = [r0, r1, r2, r3];
    }

    return { type: CLIP_PATH_TYPE.INSET, top: v0, right: v1, bottom: v2, left: v3, round };
};
